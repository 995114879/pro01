...
def t1(img):
    ...
    img1[590:1100, 600:-650, :] = img[590:1100, 600:-650, :]
    ...
    # 二值化
    _, threshold = cv.threshold(gamma_corrected, 210, 255, cv.THRESH_TOZERO_INV)
    ...
    # 二值化
    _, threshold = cv.threshold(threshold, 100, 255, cv.THRESH_BINARY)
    ...
    # 膨胀+腐蚀操作
    kernel = np.ones((5, 5), np.uint8)
    dst = cv.erode(threshold, kernel, iterations=6, borderType=cv.BORDER_REFLECT)
    ...
    cv.imwrite('./labels/new_label/Image_20240925133507624.jpg', img2)


if __name__ == '__main__':
    img = cv.imread("./datas/new_img/Image_20240925133507624.bmp")
    t1(img)
